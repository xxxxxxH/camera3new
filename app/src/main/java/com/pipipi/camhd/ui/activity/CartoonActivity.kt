...
import com.pipipi.camhd.item.CartoonItem
...
class CartoonActivity : BaseActivity(R.layout.activity_cartoon) {
        ...
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                1 -> {
                    ...
                    data.forEach {
                        adapter.add(CartoonItem(this@CartoonActivity, it))
                    }
                    recycler.layoutManager = GridLayoutManager(this@CartoonActivity, 4)
    ...
    private fun initData() {
        thread {
            val data = AssetsManager.get().getCartoons(this)
            val msg = Message()
            msg.what = 1
            msg.obj = data
